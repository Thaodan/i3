...
#define GET_PROPERTY(atom, len)  xcb_get_property(conn, false, window, atom, XCB_GET_PROPERTY_TYPE_ANY, 0, len)
    ...
    xcb_get_property_reply_t *wm_pid_reply =
        xcb_get_property_reply(conn, wm_pid_cookie, NULL);
    DLOG("MESSAGE!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!\n");
    if (wm_pid_reply != NULL && xcb_get_property_value_length(wm_pid_reply) != 0) {
        printf("WM_NAME is %i\n", wm_pid_reply->type);

        uint32_t*wm_pid = xcb_get_property_value(wm_pid_reply);
        if(wm_pid) {
        DLOG("wm pid = %i\n", *wm_pid);
        cwindow->pid = (pid_t)wm_pid;
        }
    }
    else {
        DLOG("WM pid = -1\n");
